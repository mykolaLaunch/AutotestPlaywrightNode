...
test.describe('Google Drive tests', { tag: ['@google-drive', '@regression'] }, () => {
  ...
  test('Google Drive coverage without config-based (neo4j)', async () => {
    console.info('--- Google Drive coverage (neo4j) test start');
    console.info('Action: fetch Drive file ids and compare to Neo4j DataItem.externalId.');
    const driveRepository = new GoogleDriveRepository();
    ...
    const neo4jRows = await neo4jDataItemRepository.getBySourceAndAccount('google-drive', targetEmail);
    ...
    const missingIds = driveResult.ids.filter((id) => !neo4jSet.has(id));
    const missingDetailsErrors: string[] = [];
    const missingLookupErrors: string[] = [];
    ...
    if (missingIds.length > 0) {
      ...
      missingDetailsErrors.push(
        `Missing Google Drive files in Neo4j (showing up to 10):\n${lines.join('\n')}`
      );
    ...
    expect(errors, errors.join('\n')).toHaveLength(0);
    console.info('--- Google Drive coverage (neo4j) test end');
  });
  ...
  test('Google Drive config-based external_id coverage for mykola@launchnyc.io (neo4j)', async ({ request }) => {
    console.info('--- Google Drive coverage (config-based, neo4j) test start');
    console.info('Action: load Drive instance settings and compare filtered Drive ids to Neo4j DataItem.externalId.');
    loadEnvOnce();
    ...
    const extraIds = neo4jExternalIdResult.externalIds.filter((id) => !driveSet.has(id));
    const extraDetailsErrors: string[] = [];

    if (extraIds.length > 0) {
      console.info(`Extra Drive files in Neo4j (not in configured Drive results): ${extraIds.length}`);
      const sampleIds = extraIds.slice(0, 50);
      const lookupResult = await driveRepository.getFileDetailsByIds(sampleIds, 'me');
      extraDetailsErrors.push(...lookupResult.errors);
      ...
      extraDetailsErrors.push(
        `Extra Google Drive files in Neo4j (showing up to 50):\n${lines.join('\n')}`
      );
    }
    ...
    expect(errors, errors.join('\n')).toHaveLength(0);
    console.info('--- Google Drive coverage (config-based, neo4j) test end');
  });
  ...
  test(
    'Google Drive duplicates (neo4j)',
    { tag: ['@check-duplicates', '@neo4j'] },
    async () => {
      console.info('--- Google Drive duplicates (neo4j) test start');
      console.info('Action: load Drive DataItem rows from Neo4j and check for duplicate externalId.');
      const neo4jDataItemRepository = new Neo4jDataItemRepository();
      ...
      if (duplicates.length > 0) {
        console.info(`Duplicate Drive files in Neo4j (${duplicates.length}):`);
        const lines = duplicates.map((row) => `${row.externalId} (count=${row.count})`);
        console.info(lines.join('\n'));
      }

      expect(duplicates.length, 'Duplicate Drive files found in Neo4j').toBe(0);
      console.info('--- Google Drive duplicates (neo4j) test end');
    }
  );
  ...
  test(
    'Google Drive order by modified_time (neo4j)',
    { tag: ['@order-test', '@neo4j'] },
    async () => {
      console.info('--- Google Drive ingestion order (neo4j) test start');
      console.info('Action: validate Drive modifiedTime increases while Neo4j rawVersionId decreases.');
      const neo4jDataItemRepository = new Neo4jDataItemRepository();
      const driveRepository = new GoogleDriveRepository();
      const validator = new GoogleDriveExternalIdValidator();
      ...
      const driveOrderItemsResult = validator.buildDriveModifiedOrderItemsFromNeo4j(
        neo4jRows,
        driveDetailsResult.detailsById
      );
      const orderResult = validator.validateDriveModifiedTimeRawVersionIdOrder(
        driveOrderItemsResult.items,
        minSamples
      );
      ...
      expect(errors, errors.join('\n')).toHaveLength(0);
      console.info('--- Google Drive ingestion order (neo4j) test end');
    }
  );
  ...
  test(
    'Catch Google Drive new file (neo4j)',
    { tag: ['@google-drive', '@dynamic', '@new-object-load', '@neo4j'] },
    async () => {
      console.info('--- Google Drive dynamic ingestion (neo4j) test start');
      console.info('Action: create a Drive file and poll Neo4j DataItem by externalId.');

      const driveRepository = new GoogleDriveRepository();
      const neo4jDataItemRepository = new Neo4jDataItemRepository();

      const timestamp = new Date().toISOString();
      const fileName = `PW-DRIVE-INGESTION-NEO4J ${timestamp}.txt`;
      const content = 'Playwright Google Drive ingestion test content for Neo4j dynamic check.';
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        const rows = await neo4jDataItemRepository.getBySourceAndExternalId('google-drive', fileId);
        const found = rows.length > 0;
        console.info(`Poll attempt ${attempt}/${maxAttempts}: found=${found} rows=${rows.length}`);
        attemptsLog.push({
          attempt,
      ...
      try {
        expect(matchedRows.length, 'No Neo4j DataItem row found for created Drive file. Wait time 2 minutes').toBeGreaterThan(0);

        const latestRow = matchedRows[0];
        expect(latestRow.externalId, 'Neo4j DataItem externalId mismatch.').toBe(fileId);

        console.info('--- Google Drive dynamic ingestion (neo4j) test end');
      } finally {
        if (fileId) {
          const deleteResult = await driveRepository.deleteFile(fileId);
          if (deleteResult.errors.length > 0) {
